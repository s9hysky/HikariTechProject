package com.RakuwakoVampire.bloodmemoryencounter.item.skills;
...
import com.RakuwakoVampire.bloodmemoryencounter.item.ItemBase;
import com.RakuwakoVampire.bloodmemoryencounter.util.CommonFunctions;
import com.RakuwakoVampire.bloodmemoryencounter.util.IDLSkillNBT;
import com.RakuwakoVampire.bloodmemoryencounter.util.MessageDef;
...
import static com.RakuwakoVampire.bloodmemoryencounter.util.CommonDef.G_SKY;
import static com.RakuwakoVampire.bloodmemoryencounter.util.CommonDef.TICK_PER_SECOND;
import static com.RakuwakoVampire.bloodmemoryencounter.util.CommonFunctions.SendMsgToPlayer;
import static com.RakuwakoVampire.bloodmemoryencounter.util.CommonFunctions.isShiftPressed;
import static com.RakuwakoVampire.bloodmemoryencounter.util.MessageDef.getSkillCastKey;
...
public class ItemSkillBase extends ItemBase {
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "bloodmemoryencounter.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("bloodmemoryencounter.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("bloodmemoryencounter.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("bloodmemoryencounter.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("bloodmemoryencounter.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("bloodmemoryencounter.skill.shared.lv_max") : "";

        return I18n.format("bloodmemoryencounter.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("bloodmemoryencounter.skill.shared.duration_desc", val);
    }
