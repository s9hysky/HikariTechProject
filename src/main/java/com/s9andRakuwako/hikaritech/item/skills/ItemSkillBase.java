...
import com.s9andRakuwako.hikaritech.util.IDLSkillNBT;
import com.s9andRakuwako.hikaritech.util.MessageDef;
...
import static com.s9andRakuwako.hikaritech.util.CommonDef.G_SKY;
import static com.s9andRakuwako.hikaritech.util.CommonDef.TICK_PER_SECOND;
import static com.s9andRakuwako.hikaritech.util.CommonFunctions.SendMsgToPlayer;
import static com.s9andRakuwako.hikaritech.util.CommonFunctions.isShiftPressed;
import static com.s9andRakuwako.hikaritech.util.MessageDef.getSkillCastKey;
...
public class ItemSkillBase extends ItemBase {
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "hikaritech.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("hikaritech.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("hikaritech.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("hikaritech.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("hikaritech.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("hikaritech.skill.shared.lv_max") : "";

        return I18n.format("hikaritech.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("hikaritech.skill.shared.duration_desc", val);
    }
